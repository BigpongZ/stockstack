...
const TX_KEY = "stock_transactions_v1";

function useTrades() {
  const [trades, setTrades] = useState(() => {
    try { const raw = localStorage.getItem(TX_KEY); return raw ? JSON.parse(raw) : []; }
    catch { return []; }
  });
  const saveTrades = (newTrades) => {
    setTrades(newTrades);
    try { localStorage.setItem(TX_KEY, JSON.stringify(newTrades)); } catch {}
  };
  const addTrade = (trade) => saveTrades([...trades, { ...trade, id: Date.now(), date: new Date().toLocaleDateString("th-TH") }]);
  const removeTrade = (id) => saveTrades(trades.filter(t => t.id !== id));
  return { trades, addTrade, removeTrade };
}

function calcRealizedPnL(trades, symbol, market, usdthb) {
  const symbolTrades = trades.filter(t => t.symbol === symbol && t.market === market);
  const rate = market === "US" ? usdthb : 1;
  let realized = 0;
  let buyQueue = [];
  symbolTrades.forEach(t => {
    if (t.type === "buy") {
      buyQueue.push({ price: t.price, qty: t.qty });
    } else if (t.type === "sell") {
      let remaining = t.qty;
      while (remaining > 0 && buyQueue.length > 0) {
        const oldest = buyQueue[0];
        const matched = Math.min(oldest.qty, remaining);
        realized += (t.price - oldest.price) * matched * rate;
        oldest.qty -= matched;
        remaining -= matched;
...
function TradePanel({ stock, trades, onAddTrade, onRemoveTrade, onClose, usdthb }) {
  const [type, setType] = useState("buy");
  const [price, setPrice] = useState("");
  const [qty, setQty] = useState("");
  const [date, setDate] = useState(new Date().toISOString().split("T")[0]);

  const symbolTrades = trades.filter(t => t.symbol === stock.symbol && t.market === stock.market);
  const realized = calcRealizedPnL(trades, stock.symbol, stock.market, usdthb);
  const rate = stock.market === "US" ? usdthb : 1;
  const currency = stock.market === "TH" ? "THB" : "USD";

  const handleAdd = () => {
    if (!price || !qty) return;
    onAddTrade({ symbol: stock.symbol, market: stock.market, type, price: parseFloat(price), qty: parseFloat(qty), dateStr: date });
    setPrice(""); setQty("");
  };

  const inp = { background: T.bg, border: `1px solid ${T.border}`, color: T.text, borderRadius: "8px", padding: "8px 10px", fontSize: "0.88rem", boxSizing: "border-box" };

  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: "1rem" }}>
      <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: "16px", width: "100%", maxWidth: "480px", maxHeight: "90vh", overflow: "auto", boxShadow: "0 20px 60px rgba(0,0,0,0.15)" }}>
        <div style={{ padding: "1.25rem 1.5rem", borderBottom: `1px solid ${T.border}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>
            <span style={{ fontFamily: "monospace", fontWeight: 700, fontSize: "1.1rem", color: T.text }}>{stock.symbol}</span>
            <span style={{ marginLeft: "0.5rem", color: T.textMuted, fontSize: "0.8rem" }}>บันทึกการซื้อขาย</span>
          </div>
          <button onClick={onClose} style={{ background: "none", border: "none", color: T.textMuted, cursor: "pointer", fontSize: "1.3rem" }}>✕</button>
        </div>
        ...
        <div style={{ padding: "1.25rem" }}>
          {realized !== 0 && (
            <div style={{ background: realized >= 0 ? T.greenBg : T.redBg, border: `1px solid ${realized >= 0 ? T.greenBorder : T.redBorder}`, borderRadius: "10px", padding: "0.85rem 1rem", marginBottom: "1rem" }}>
              <div style={{ fontSize: "0.72rem", color: T.textMuted, marginBottom: "0.25rem" }}>กำไรสะสม (Realized P&L)</div>
              <div style={{ fontFamily: "monospace", fontWeight: 700, fontSize: "1.2rem", color: realized >= 0 ? T.green : T.red }}>
                {realized >= 0 ? "+" : ""}{realized.toFixed(2)} THB
              </div>
            </div>
          )}

          <div style={{ marginBottom: "1rem" }}>
            <div style={{ fontSize: "0.72rem", color: T.textMuted, marginBottom: "0.5rem", fontWeight: 600 }}>บันทึก transaction ใหม่</div>
            <div style={{ display: "flex", gap: "0.5rem", marginBottom: "0.5rem" }}>
              {["buy", "sell"].map(t => (
                <button key={t} onClick={() => setType(t)} style={{ flex: 1, padding: "8px", borderRadius: "8px", fontWeight: 700, cursor: "pointer", fontSize: "0.85rem", border: type === t ? `2px solid ${t === "buy" ? T.green : T.red}` : `1px solid ${T.border}`, background: type === t ? (t === "buy" ? T.greenBg : T.redBg) : "transparent", color: type === t ? (t === "buy" ? T.green : T.red) : T.textMuted }}>
                  {t === "buy" ? "🟢 ซื้อ" : "🔴 ขาย"}
                </button>
              ))}
            </div>
            <div style={{ display: "flex", gap: "0.5rem", marginBottom: "0.5rem" }}>
              <input placeholder={`ราคา (${currency})`} value={price} onChange={e => setPrice(e.target.value)} type="number" style={{ ...inp, flex: 1 }} />
              <input placeholder="จำนวนหุ้น" value={qty} onChange={e => setQty(e.target.value)} type="number" style={{ ...inp, flex: 1 }} />
            </div>
            <input type="date" value={date} onChange={e => setDate(e.target.value)} style={{ ...inp, width: "100%", marginBottom: "0.5rem" }} />
            {price && qty && (
              <div style={{ fontSize: "0.78rem", color: T.textMuted, marginBottom: "0.5rem" }}>
                มูลค่า: {(parseFloat(price) * parseFloat(qty)).toFixed(2)} {currency}
                {stock.market === "US" && ` ≈ ${(parseFloat(price) * parseFloat(qty) * usdthb).toFixed(0)} THB`}
              </div>
            )}
            <button onClick={handleAdd} style={{ width: "100%", background: type === "buy" ? T.green : T.red, color: "#fff", border: "none", borderRadius: "8px", padding: "10px", fontWeight: 700, cursor: "pointer", fontSize: "0.88rem" }}>
              + บันทึก{type === "buy" ? "การซื้อ" : "การขาย"}
            </button>
          </div>

          {symbolTrades.length > 0 && (
            <div>
              <div style={{ fontSize: "0.72rem", color: T.textMuted, marginBottom: "0.5rem", fontWeight: 600 }}>ประวัติ ({symbolTrades.length} รายการ)</div>
              {[...symbolTrades].reverse().map(t => (
                <div key={t.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "0.6rem 0.75rem", marginBottom: "0.4rem", background: t.type === "buy" ? T.greenBg : T.redBg, borderRadius: "8px", border: `1px solid ${t.type === "buy" ? T.greenBorder : T.redBorder}` }}>
                  <div>
                    <span style={{ fontWeight: 700, color: t.type === "buy" ? T.green : T.red, fontSize: "0.8rem" }}>{t.type === "buy" ? "ซื้อ" : "ขาย"}</span>
                    <span style={{ color: T.text, fontFamily: "monospace", fontSize: "0.82rem", marginLeft: "0.5rem" }}>{t.qty} หุ้น @ {t.price} {currency}</span>
                  </div>
                  <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
                    <span style={{ color: T.textFaint, fontSize: "0.7rem" }}>{t.date}</span>
                    <button onClick={() => onRemoveTrade(t.id)} style={{ background: "none", border: "none", color: T.textFaint, cursor: "pointer", fontSize: "0.85rem" }}>✕</button>
                  </div>
                </div>
              ))}
            </div>
...
function StockCard({ item, onRemove, onAnalyze, onUpdate, onShowNews, onShowTrades, refreshTick, trades, usdthb }) {
  const [quote, setQuote] = useState(null);
    ...
    <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: "14px", padding: "1.1rem", boxShadow: T.shadow, transition: "box-shadow 0.2s" }}
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
        ...
        <div style={{ display: "flex", gap: "0.35rem", flexWrap: "wrap", justifyContent: "flex-end" }}>
          ...
          <button onClick={() => onShowTrades(item)} style={{ background: item.market === "TH" ? T.greenBg : "#fef9c318", border: `1px solid ${item.market === "TH" ? T.greenBorder : "#fde68a"}`, color: item.market === "TH" ? T.green : T.gold, borderRadius: "6px", padding: "4px 8px", fontSize: "0.68rem", cursor: "pointer", fontWeight: 600 }}>ซื้อขาย</button>
      ...
      {(() => { const r = calcRealizedPnL(trades, item.symbol, item.market, usdthb); return r !== 0 ? (
        <div style={{ marginTop: "0.3rem", fontSize: "0.75rem", color: r >= 0 ? T.green : T.red, fontFamily: "monospace" }}>
          กำไรสะสม: {r >= 0 ? "+" : ""}{r.toFixed(0)} THB
        </div>
      ) : null; })()}
      {item.target && (
...
function PortfolioSummary({ items, usdthb, trades }) {
  ...
  const totalRealized = items.reduce((sum, item) => sum + calcRealizedPnL(trades, item.symbol, item.market, usdthb), 0);
  ...
  return (
    <div style={{ background: up ? T.greenBg : T.redBg, border: `1px solid ${up ? T.greenBorder : T.redBorder}`, borderRadius: "12px", padding: "1rem 1.25rem", marginBottom: "1.25rem", boxShadow: T.shadow }}>
      ...
      {totalRealized !== 0 && (
        <div style={{ marginTop: "0.75rem", paddingTop: "0.75rem", borderTop: `1px solid ${up ? T.greenBorder : T.redBorder}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div style={{ fontSize: "0.72rem", color: T.textMuted }}>กำไรสะสมจากการขาย</div>
          <div style={{ fontFamily: "monospace", fontWeight: 700, color: totalRealized >= 0 ? T.green : T.red, fontSize: "1rem" }}>
            {totalRealized >= 0 ? "+" : ""}{totalRealized.toFixed(0)} THB
          </div>
        </div>
      )}
    </div>
...
export default function App() {
  ...
  const [showingTrades, setShowingTrades] = useState(null);
  ...
  return (
    <div style={{ minHeight: "100vh", background: isDark ? THEMES.dark.bg : THEMES.light.bg, fontFamily: "system-ui, sans-serif", color: isDark ? THEMES.dark.text : THEMES.light.text }}>
      ...
      <div style={{ borderBottom: `1px solid ${T.border}`, padding: "0.75rem 1rem", display: "flex", justifyContent: "space-between", alignItems: "center", position: "sticky", top: 0, background: isDark ? THEMES.dark.header : THEMES.light.header, zIndex: 100, boxShadow: "0 1px 4px rgba(0,0,0,0.06)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "0.4rem" }}>
          <span style={{ fontSize: "1.1rem" }}>◈</span>
          <span style={{ fontWeight: 700, fontSize: "0.95rem", color: isDark ? THEMES.dark.text : THEMES.light.text }}>StockWatch</span>
          <span style={{ color: T.teal, fontSize: "0.65rem", fontWeight: 600, background: T.tealBg, padding: "1px 6px", borderRadius: "20px", border: `1px solid ${T.tealBorder}` }}>AI</span>
        </div>
        <div style={{ display: "flex", gap: "0.35rem", alignItems: "center" }}>
          <button onClick={() => setIsDark(d => !d)} title="เปลี่ยน theme" style={{ background: "transparent", border: `1px solid ${T.border}`, color: T.textMuted, borderRadius: "7px", padding: "5px 8px", cursor: "pointer", fontSize: "0.82rem", lineHeight: 1 }}>
            {isDark ? "☀️" : "🌙"}
          </button>
          <button onClick={handleRefresh} title="รีเฟรชราคา" style={{ background: "transparent", border: `1px solid ${T.border}`, color: T.textMuted, borderRadius: "7px", padding: "5px 8px", cursor: "pointer", fontSize: "0.82rem", lineHeight: 1 }}>
            {refreshing ? "⟳" : "↻"}
          </button>
          <button onClick={handleExport} title="Export CSV" style={{ background: "transparent", border: `1px solid ${T.border}`, color: T.textMuted, borderRadius: "7px", padding: "5px 8px", cursor: "pointer", fontSize: "0.75rem", lineHeight: 1 }}>
            ⬇CSV
          </button>
          <button onClick={() => setShowAdd(true)} style={{ background: T.teal, color: "#fff", border: "none", borderRadius: "7px", padding: "6px 12px", fontWeight: 700, cursor: "pointer", fontSize: "0.82rem", lineHeight: 1 }}>+ เพิ่ม</button>
        </div>
      </div>
      ...
      <div style={{ maxWidth: "920px", margin: "0 auto", padding: "1.25rem 1rem" }}>
        ...
        <PortfolioSummary items={filtered} usdthb={usdthb} trades={trades} />
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(265px, 1fr))", gap: "0.85rem" }}>
          {filtered.map(item => (
            <StockCard key={item.id} item={item} onRemove={remove} onAnalyze={setAnalyzing} onUpdate={update} onShowNews={setShowingNews} onShowTrades={setShowingTrades} refreshTick={refreshTick} trades={trades} usdthb={usdthb} />
          ))}
        </div>
      </div>
      ...
      {showingTrades && <TradePanel stock={showingTrades} trades={trades} onAddTrade={addTrade} onRemoveTrade={removeTrade} onClose={() => setShowingTrades(null)} usdthb={usdthb} />}
    </div>
  );
}
