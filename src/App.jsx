...
async function analyzeStock(stockInfo, analysisType) {
  const portfolioContext = stockInfo.shares && stockInfo.avgCost
    ? `นักลงทุนถือหุ้น ${stockInfo.shares} หุ้น ราคาเฉลี่ย ${stockInfo.avgCost} ${stockInfo.currency} กำไร/ขาดทุนปัจจุบัน: ${((stockInfo.price - stockInfo.avgCost) * stockInfo.shares).toFixed(2)} ${stockInfo.currency} วิเคราะห์ว่าควรถือต่อหรือขาย`
    : "";
  const prompts = {
...
function PnL({ shares, avgCost, currentPrice, currency }) {
  ...
  useEffect(() => {
    if (currency === "USD") fetchUSDTHB().then(setUsdthb);
  }, [currency]);
  ...
  return (
    <div style={{ marginTop: "0.4rem", background: up ? "#00d4aa11" : "#ff4d6d11", border: `1px solid ${up ? "#00d4aa33" : "#ff4d6d33"}`, borderRadius: "8px", padding: "0.5rem 0.75rem" }}>
      ...
      {pnlTHB !== null && (
        <div style={{ fontFamily: "monospace", fontSize: "0.8rem", color: up ? "#00d4aa99" : "#ff4d6d99", marginTop: "0.15rem" }}>
          {up ? "+" : ""}{pnlTHB.toFixed(0)} THB
          <span style={{ color: "#484f58", marginLeft: "0.4rem" }}>(1 USD = {usdthb.toFixed(2)} THB)</span>
        </div>
      )}
    </div>
  );
...
function NewsPanel({ stock, onClose }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/news?symbol=${encodeURIComponent(stock.symbol)}&name=${encodeURIComponent(name)}`)
      .then(r => r.json())
  ...
  const formatDate = (dateStr) => {
    if (!dateStr) return "";
    try {
      return new Date(dateStr).toLocaleDateString("th-TH", { day: "numeric", month: "short", hour: "2-digit", minute: "2-digit" });
    } catch { return ""; }
  };
      ...
      <div style={{ background: "#0d1117", border: "1px solid #21262d", borderRadius: "16px", width: "100%", maxWidth: "600px", maxHeight: "90vh", overflow: "auto", boxShadow: "0 0 60px rgba(0,212,170,0.15)" }}>
        ...
        <div style={{ padding: "1.25rem" }}>
          {loading ? (
            ...
            <div>
              {news.map((item, i) => (
                <a key={i} href={item.link} target="_blank" rel="noopener noreferrer" style={{ display: "block", padding: "0.75rem", marginBottom: "0.5rem", background: "#161b22", borderRadius: "8px", border: "1px solid #21262d", textDecoration: "none" }}>
              ...
              {!summary && (
                <button onClick={summarizeNews} disabled={summarizing} style={{ width: "100%", background: "#00d4aa18", border: "1px solid #00d4aa44", color: "#00d4aa", borderRadius: "8px", padding: "10px", fontWeight: 600, cursor: "pointer", fontSize: "0.85rem", marginTop: "0.5rem" }}>
                  {summarizing ? "AI กำลังสรุปข่าว..." : "✦ ให้ AI สรุปข่าวทั้งหมด"}
                </button>
              )}
              {summary && (
                <div style={{ background: "#161b22", borderRadius: "10px", padding: "1rem", border: "1px solid #00d4aa33", marginTop: "0.75rem" }}>
...
function AddStockModal({ onAdd, onClose }) {
  ...
  const search = async () => {
    ...
    if (basket.find(b => b.symbol === sym && b.market === market)) {
      setError("มีหุ้นนี้ในตะกร้าแล้ว"); return;
    }
    setLoading(true); setError("");
    const q = await fetchQuote(sym, market);
    if (q) {
      setBasket(prev => [...prev, { ...q, shares: null, avgCost: null, target: null, note: "" }]);
      setSymbol("");
  ...
  const startEdit = (idx) => {
    ...
    setAvgCost(item.avgCost || "");
    setTarget(item.target || "");
    setNote(item.note || "");
  };

  const saveEdit = () => {
    setBasket(prev => prev.map((item, i) => i === editIdx ? {
      ...item,
    ...
    setEditIdx(null);
    setShares(""); setAvgCost(""); setTarget(""); setNote("");
  };
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.85)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: "1rem" }}>
      <div style={{ background: "#0d1117", border: "1px solid #21262d", borderRadius: "16px", width: "100%", maxWidth: "480px", padding: "1.5rem", maxHeight: "92vh", overflow: "auto" }}>
        ...
        <div style={{ display: "flex", gap: "0.5rem", marginBottom: "1rem" }}>
          {["TH", "US"].map(m => (
            <button key={m} onClick={() => { setMarket(m); setError(""); }} style={{ flex: 1, padding: "8px", borderRadius: "8px", fontWeight: 600, cursor: "pointer", border: market === m ? "1px solid #00d4aa" : "1px solid #30363d", background: market === m ? "#00d4aa18" : "transparent", color: market === m ? "#00d4aa" : "#8b949e" }}>
              {m === "TH" ? "🇹🇭 ไทย (SET)" : "🇺🇸 US"}
            </button>
          ))}
        </div>
        ...
        <div style={{ display: "flex", gap: "0.5rem", marginBottom: "0.5rem" }}>
          <input placeholder={market === "TH" ? "เช่น PTT, KBANK, AOT" : "เช่น AAPL, NVDA, TSLA"} value={symbol} onChange={e => setSymbol(e.target.value.toUpperCase())} onKeyDown={e => e.key === "Enter" && search()} style={{ flex: 1, background: "#161b22", border: "1px solid #30363d", color: "#e6edf3", borderRadius: "8px", padding: "8px 12px", fontSize: "0.9rem", fontFamily: "monospace" }} />
          <button onClick={search} style={{ background: "#00d4aa", color: "#000", border: "none", borderRadius: "8px", padding: "8px 16px", fontWeight: 700, cursor: "pointer" }}>{loading ? "..." : "+ เพิ่ม"}</button>
        </div>
        ...
        {basket.length > 0 && (
          <div style={{ marginBottom: "1rem" }}>
            <div style={{ fontSize: "0.72rem", color: "#8b949e", marginBottom: "0.5rem" }}>ตะกร้า ({basket.length} หุ้น)</div>
            {basket.map((item, idx) => (
              <div key={idx} style={{ background: "#161b22", border: "1px solid #21262d", borderRadius: "10px", padding: "0.75rem", marginBottom: "0.5rem" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <div>
                    <span style={{ fontFamily: "monospace", fontWeight: 700, color: "#e6edf3" }}>{item.symbol}</span>
                    <span style={{ marginLeft: "0.5rem", color: "#00d4aa", fontFamily: "monospace", fontSize: "0.9rem" }}>{item.price?.toFixed(2)} {item.currency}</span>
                    {item.shares && <span style={{ marginLeft: "0.5rem", color: "#8b949e", fontSize: "0.75rem" }}>{item.shares} หุ้น @ {item.avgCost}</span>}
                  </div>
                  <div style={{ display: "flex", gap: "0.4rem" }}>
                    <button onClick={() => startEdit(idx)} style={{ background: "transparent", border: "1px solid #30363d", color: "#8b949e", borderRadius: "5px", padding: "3px 8px", fontSize: "0.7rem", cursor: "pointer" }}>✏️</button>
                    <button onClick={() => removeFromBasket(idx)} style={{ background: "transparent", border: "none", color: "#ff4d6d", cursor: "pointer", fontSize: "0.9rem" }}>✕</button>
                  </div>
                </div>
                {editIdx === idx && (
                  <div style={{ marginTop: "0.6rem", borderTop: "1px solid #30363d", paddingTop: "0.6rem" }}>
                    <div style={{ display: "flex", gap: "0.5rem", marginBottom: "0.4rem" }}>
                      <input placeholder="จำนวนหุ้น" value={shares} onChange={e => setShares(e.target.value)} type="number" style={{ flex: 1, background: "#0d1117", border: "1px solid #30363d", color: "#e6edf3", borderRadius: "6px", padding: "5px 8px", fontSize: "0.82rem", boxSizing: "border-box" }} />
                      <input placeholder="ราคาที่ซื้อเฉลี่ย" value={avgCost} onChange={e => setAvgCost(e.target.value)} type="number" style={{ flex: 1, background: "#0d1117", border: "1px solid #30363d", color: "#e6edf3", borderRadius: "6px", padding: "5px 8px", fontSize: "0.82rem", boxSizing: "border-box" }} />
                    </div>
                    <input placeholder="ราคาเป้าหมาย" value={target} onChange={e => setTarget(e.target.value)} type="number" style={{ width: "100%", background: "#0d1117", border: "1px solid #30363d", color: "#e6edf3", borderRadius: "6px", padding: "5px 8px", fontSize: "0.82rem", marginBottom: "0.4rem", boxSizing: "border-box" }} />
                    <textarea placeholder="โน้ต..." value={note} onChange={e => setNote(e.target.value)} rows={2} style={{ width: "100%", background: "#0d1117", border: "1px solid #30363d", color: "#e6edf3", borderRadius: "6px", padding: "5px 8px", fontSize: "0.82rem", resize: "none", marginBottom: "0.4rem", boxSizing: "border-box" }} />
        ...
        {basket.length > 0 && (
          <button onClick={handleAddAll} style={{ width: "100%", background: "#00d4aa", color: "#000", border: "none", borderRadius: "8px", padding: "11px", fontWeight: 700, cursor: "pointer", fontSize: "0.9rem" }}>
...
function StockCard({ item, onRemove, onAnalyze, onUpdate, onShowNews }) {
  const [quote, setQuote] = useState(null);
    ...
    <div style={{ background: "#0d1117", border: "1px solid #21262d", borderRadius: "14px", padding: "1.1rem", transition: "border-color 0.2s" }}
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
        ...
        <div style={{ display: "flex", gap: "0.4rem" }}>
          ...
          <button onClick={() => onShowNews(item)} style={{ background: "#58a6ff18", border: "1px solid #58a6ff44", color: "#58a6ff", borderRadius: "6px", padding: "4px 8px", fontSize: "0.7rem", cursor: "pointer", fontWeight: 600 }}>ข่าว</button>
...
function PortfolioSummary({ items }) {
  ...
  useEffect(() => {
    ...
    if (items.some(i => i.market === "US" && i.shares && i.avgCost)) {
      fetchUSDTHB().then(setUsdthb);
    }
  }, [items]);
  ...
  return (
    <div style={{ background: "#0d1117", border: `1px solid ${up ? "#00d4aa33" : "#ff4d6d33"}`, borderRadius: "12px", padding: "1rem 1.25rem", marginBottom: "1.25rem" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "0.6rem" }}>
        <div style={{ fontSize: "0.72rem", color: "#8b949e" }}>สรุปพอร์ต ({portfolioItems.length} หุ้น)</div>
        {hasUS && <div style={{ fontSize: "0.65rem", color: "#484f58" }}>USD/THB = {usdthb.toFixed(2)}</div>}
      </div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "0.75rem" }}>
        <div>
          <div style={{ fontSize: "0.7rem", color: "#8b949e" }}>ต้นทุนรวม</div>
          <div style={{ fontFamily: "monospace", color: "#e6edf3", fontWeight: 600 }}>{totalCostTHB.toFixed(0)}</div>
          <div style={{ fontSize: "0.65rem", color: "#484f58" }}>THB</div>
        </div>
        <div>
          <div style={{ fontSize: "0.7rem", color: "#8b949e" }}>มูลค่าปัจจุบัน</div>
          <div style={{ fontFamily: "monospace", color: "#e6edf3", fontWeight: 600 }}>{totalValueTHB.toFixed(0)}</div>
          <div style={{ fontSize: "0.65rem", color: "#484f58" }}>THB</div>
        </div>
        <div style={{ textAlign: "right" }}>
          ...
          <div style={{ fontSize: "0.8rem", color: up ? "#00d4aa99" : "#ff4d6d99" }}>({up ? "+" : ""}{totalPct}%)</div>
        </div>
      </div>
    </div>
  );
    ...
    <div style={{ minHeight: "100vh", background: "#010409", fontFamily: "system-ui, sans-serif", color: "#e6edf3" }}>
      ...
      <div style={{ maxWidth: "900px", margin: "0 auto", padding: "1.25rem 1rem" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: "0.85rem" }}>
          {filtered.map(item => (
            <StockCard key={item.id} item={item} onRemove={remove} onAnalyze={setAnalyzing} onUpdate={update} onShowNews={setShowingNews} />
          ))}
        </div>
      </div>
